...
//---------------------------------------------------------------------------------------------------------------
    public ArrayList<Licencia> consultarSoft(String consulta) throws SQLException 
    {   
        ...
        try {
            while (rs.next()) {
                //Asigna los valores de Id y Nombre del software que se va recorriendo
                int id_soft = rs.getInt("codigo");
                String nombre_soft = rs.getString("nombre");                
                //carga un objeto de tipo Licencia
                lista.add(new Licencia(id_soft, nombre_soft));
            }
        } catch (SQLException e) {
            mensaje = "La sentencia preparada no se ha podido cerrar correctamente " + e.getMessage();
        }
    ...
    public ArrayList<Licencia> consultarSoft2(String consulta) throws SQLException 
    {   
        ArrayList<Licencia> lista = new ArrayList<Licencia>();
        PreparedStatement ps = con.prepareStatement(consulta);
        ResultSet rs = ps.executeQuery();
        try {
            while (rs.next()) {
                //Asigna los valores de Id y Nombre del software que se va recorriendo
                int id_soft = rs.getInt("idSoftware");
                String nombre_soft = rs.getString("NombreSoft");                
                //carga un objeto de tipo Licencia
                lista.add(new Licencia(id_soft, nombre_soft));
            }
        } catch (SQLException e) {
...
public int conectarBDErpProcCrea(java.lang.String bloque) {
  ...
  try {
            ...
            CallableStatement cstmt = con.prepareCall("{call ServiOsirirs.dbo.AsignaTareas (?)}");
            cstmt.setString(1, bloque);            
